...
class _UpScale(nn.Sequential):
    def __init__(self, input_features, output_features):
        ...
        self.add_module('conv2_', Conv2d(input_features, output_features * 4,
                                         kernel_size=3))
        self.add_module('bathNorm2_', nn.BatchNorm2d(output_features * 4))
        self.add_module('leakyrelu', nn.LeakyReLU(0.1, inplace=True))
...
class Reshape(nn.Module):
    ...
    def forward(self, input):
        output = input.view(-1, 1024, 8, 8)  # channel * 4 * 4
...
class Autoencoder(nn.Module):
    def __init__(self):
        ...
        self.decoder_A = nn.Sequential(
            ...
            nn.BatchNorm2d(32),
            nn.ReLU(),           
            Conv2d(32, 3, kernel_size=5, padding=1),
            nn.BatchNorm2d(3),
            nn.Sigmoid(),
        )
    ...
    def forward(self, x, select='A'):
        if select == 'A':
            ...
            # out = self.decoder_A[0](out)
            # out = self.decoder_A[1](out)
            # out = self.decoder_A[2](out)
            # out = self.decoder_A[3](out)
            # out = self.decoder_A[4](out)
        else:
            out = self.encoder(x)
